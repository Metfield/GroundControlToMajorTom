...
////// Specular:
                ...
                float4 node_9799 = _Time + _TimeEditor;
                float2 node_4572 = (i.uv0+node_9799.g*float2(0.00015,0)); // Cloud speed
...
/////// Diffuse:
                ...
                float3 diffuseColor = lerp((node_6971.rgb*(lerp((_Albedocolor.rgb*_Albedo_var.rgb),float3(_Cloudstexture_var.a,_Cloudstexture_var.a,_Cloudstexture_var.a),_Clouds_var)+_Clouds_var)),(_Ambiantatmospherecolor.rgb*_Ambiantatmosphereintensity),(pow(node_7821,2.0)+0.0));
...
////// Specular:
                float NdotL = max(0, dot( normalDirection, lightDirection ));
                float4 node_6354 = _Time + _TimeEditor;
                float2 node_4572 = (i.uv0+node_6354.g*float2(0.00015,0)); // Cloud speed
                float4 _Cloudstexture_var = tex2D(_Cloudstexture,TRANSFORM_TEX(node_4572, _Cloudstexture));
                float node_2618 = (1.0 - _Cloudstexture_var.a);
                float4 _Specular_var = tex2D(_Specular,TRANSFORM_TEX(i.uv0, _Specular));
                float3 specularColor = (node_2618*(_Specularcolor.rgb*_Specular_var.rgb));
